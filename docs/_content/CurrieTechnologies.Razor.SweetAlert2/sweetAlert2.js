...
const flatten = arr => arr.reduce((flat, next) => flat.concat(Array.isArray(next) ? flatten(next) : next), []); //arr.flat(Infinity)


function dispatchQueueResult(requestId, result) {
  const queueResult = result;
  queueResult.value = queueResult.value !== undefined ? flatten(queueResult.value).map(v => v !== undefined ? getStringVersion(v) : undefined) : undefined;
  queueResult.dismiss = queueResult.dismiss !== undefined ? getEnumNumber(queueResult.dismiss) : undefined;
...
function dispatchQueuePreConfirm(requestId, inputValue) {
  const valArray = Array.isArray(inputValue) ? inputValue : [inputValue];
  return DotNet.invokeMethodAsync(namespace, "ReceivePreConfirmQueueInput", requestId, valArray.map(v => getStringVersion(v)));
}
...
function getSwalSettingsFromPoco(settings, requestId, isQueue) {
  const swalSettings = cleanSettings(settings);

  if (settings.preConfirm) {
    swalSettings.preConfirm = isQueue ? inputValue => dispatchQueuePreConfirm(requestId, inputValue) : inputValue => dispatchPreConfirm(requestId, inputValue);
  } else {
...
razorSwal.Queue = (requestId, optionIds, steps) => {
  const arrSwalSettings = optionIds.map((optionId, i) => getSwalSettingsFromPoco(steps[i], optionId, true));
  sweetalert2_all_default().queue(arrSwalSettings).then(result => {
...
razorSwal.CloseResult = result => {
  ...
  sweetalert2_all_default().close({ ...result,
...
razorSwal.GetQueueStep = () => {
  return sweetalert2_all_default().getQueueStep();
};

razorSwal.InsertQueueStep = (requestId, step, index) => {
  const stepSettings = getSwalSettingsFromPoco(step, requestId, true);
  return sweetalert2_all_default().insertQueueStep(stepSettings, index);
};

razorSwal.DeleteQueueStep = index => {
  sweetalert2_all_default().deleteQueueStep(index);
};
